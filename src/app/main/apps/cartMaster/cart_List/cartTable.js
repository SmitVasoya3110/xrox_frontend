...
import CustomeTablePagination from '../../../Components/CustomeTablePagination'
...
const EnhancedTable = ({ columns, data, index, size, funcSetIndex, funcsetSize }) => {
	const {
		getTableProps,
		headerGroups,
		prepareRow,
		...
		hooks => {
			hooks.allColumns.push(_columns => [
				// Let's make a column for selection
				{
					...
					Cell: ({ row }) => (
						<div>
							{row.original.return_status === 1 ? "Return" :
	...
	return (
		<div className="flex flex-col min-h-full sm:border-1 sm:rounded-16 overflow-hidden rounded-16"
		style={{
			"borderRadius": "20px",
