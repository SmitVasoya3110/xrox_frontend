...
import ReactTable from "./userTable";
import MultiSelectMenu from "./userMenu";

function UserTableHead(props) {
  const dispatch = useDispatch();

  const initParams = useSelector(({ user }) => user.initParams);
  ...
  useEffect(() => {
    if (!_.isMatch(params, initParams)) {
  ...
  return (
    <FuseAnimate animation="transition.slideUpIn" delay={300}>
      <ReactTable
        ...
        // onRowClick={() => {}}
        onRowClick={(ev, row) => {
          // navigate(`/view-project/${row.original.public_id}`);
        }}
        params={params}
        setParams={setParams}
        totalCount={data?.totalCountGG || 10}
      />
    </FuseAnimate>
  );
}
